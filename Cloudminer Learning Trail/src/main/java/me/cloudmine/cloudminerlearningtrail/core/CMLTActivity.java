...
import me.cloudmine.cloudminerlearningtrail.LearningTrailApplication;
...
public abstract class CMLTActivity extends ActionBarActivity implements CMLTSwap {
    ...
    private Map<Class<? extends CMLTFragment>, WeakReference<? extends CMLTFragment>> mFragmentCache;
    //Prevents the need to keep recreating the mPrams
    private static ActionBar.LayoutParams mParams;
    ...
    protected CMLTActivity() {
        mFragmentCache = new HashMap<Class<? extends CMLTFragment>, WeakReference<? extends CMLTFragment>>();
    }
    ...
    public void switchFragment(Class<? extends CMLTFragment> fragment) {
        try {
            ...
            CMLTFragment cloudmineFragment = null;
            if (mFragmentCache.containsKey(fragment)) {
                cloudmineFragment = mFragmentCache.get(fragment).get();
            }
            if(cloudmineFragment == null) {
                cloudmineFragment = fragment.newInstance();
                mFragmentCache.put(fragment, new WeakReference<CMLTFragment>(cloudmineFragment));
            }
